...
# 0.102版本号接口文档
	更新日志:
	- 对比 0.101：补充 `GetOpponentCardInHard` / `GetEnergy` / `GetChildCardList` 常用请求说明
	- 补全 `GetOpponentCardInHard`、`GetEnergy`、`GetChildCardList`、`ActiveChildCard`、`SelectWeather` 的 action_code 锚点链接
	- 新增 `ActiveChildCard` 与 `SelectWeather` 阶段说明，补充阶段流程与结果数据格式
	- 修正 `GetChildCardList` 结果类型为 `[]ChildCardDto`
	- 扩展 `CardCalc` 文档：补充换牌→行动→法术结算流程、`Combat.Result` 提交说明、`Interrupt` / `AnimationPlayEnd` 交互
	- 更新 `CardDto` 为 `buff_list`，并新增 `BuffDto` / `ChildCardDto` / `ChildState` 字段说明
	- 补充 `CombatDto.SelectCard` 和 `Behavior.SwitchCard` 文档说明
